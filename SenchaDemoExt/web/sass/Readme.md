...
folder are mapped to the application's root namespace, 'SenchaDemoExt'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "SenchaDemoExt"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'SenchaDemoExt' in the same way
as `"SenchaDemoExt/sass/src"`.
